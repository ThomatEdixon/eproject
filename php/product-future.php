...
$product = mysqli_query($conn, "SELECT product.productid , product.name , product.price , product.brandid , product.typeid , product.categoryid, product.image , product.ProductDescription, FROM product inner join category  on product.categoryid = category.categoryid   WHERE category.namecategory = 'Future' LIMIT $start,$limit");
